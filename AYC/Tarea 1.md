...
Sé de ellos por el famoso problema de P vs NP y me informé de él un poco con videos de YouTube e Internet.
## 2.
- **P (Tiempo Plolinomial)**: problemas que se pueden resolver mediante una máquina de Turing determinista y esta acotado por un polinomio $O(n^k)$. Son problemas que consideramos eficientes o tratables. 
	- UN ejemplo encontrar el camino más corto, que tiene una cota $O(|E|+|V|\log|V|)$.
...
- **REC (Lenguajes Recursivos / Problemas Decidibles)**: problemas en los que existe un algoritmo (máquina de Turing) capaz de responder "Si" o "No" de manera matemática y en un número finito de pasos. 
	- Un ejemplo es analizar si una cadena de texto pertenece a un lenguaje libre de contexto.
...
| --------- | -------------------- | ------------------------ | ------------------------------------------- | ------------------------------- |
| **P**     | Complejidad temporal | Determinista y eficiente | Cota $O(n^k)$                               | Camino más corto                |
...
1. **Herramienta utilizada**: Gemini (Google)
2. **Propósito del uso**: Búsqueda y consulta de conceptos sobre las definiciones formales de las clases de complejidad y computabilidad (P, NP, REC, RE), y la obtención de ejemplos de problemas para cada una de ellas.
3. **Prompts o instrucciones proporcionadas**: Se introdujo el enunciado del apartado 2 indicándole la explicación de las clases P, NP, RE y REC por tratarse de conceptos conocidos antes en asignaturas previas.
4. **Uso del contenido generado**: La IA devolvió definiciones formales y se redactaron las explicaciones finales con palabras propias, asociando los conceptos a las definiciones formales.
5. **Revisión y edición**: Los ejemplos generados fueron sustituidos por problemas estudiados o conocidos.